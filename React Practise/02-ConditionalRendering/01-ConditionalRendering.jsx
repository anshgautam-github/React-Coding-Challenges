    ...
    <div className="w-full h-screen flex items-center justify-center gap-10 bg-zinc-200">
      ...
      <div key={index} className="w-60 bg-zinc-100 rounded-md overflow-hidden">
          <div className="w-full h-32 bg-zinc-300">
            <img className="w-full h-full object-cover"src={item.image} alt="card-image"/>
          </div>
          <div className="w-full px-3 py-4">
            <h2 className="font-semibold text-2xl">{item.name}</h2>
            <p className="text-md mt-5">{item.desc}</p>
            <button className={`px-4 py-3 ${item.inStock?"bg-green-300":"bg-red-400"} rounded text-xs mt-3`}>
